import { ParkingSettings, PaymentMethod, POSTerminalProvider, MonthlyContract } from '../types';
...
/**
 * Helper to parse schedule strings like "20:00 a 08:00 hrs" or "20:00-08:00"
 */
export function parseScheduleHours(
  scheduleStr?: string,
  defaultStartHour: number = 20,
  defaultEndHour: number = 8
): { startMinutes: number; endMinutes: number } {
  if (scheduleStr) {
    try {
      const timeMatches = scheduleStr.match(/\b(\d{1,2}):(\d{2})\b/g);
  ...
  return {
    startMinutes: defaultStartHour * 60,
    endMinutes: defaultEndHour * 60,
  };
}

/**
 * Checks if a monthly/weekly contract is currently active right now based on dates and schedule (diurno, nocturno, 24/7, semanal).
 */
export function isContractActiveNow(
  contract?: MonthlyContract | null,
  currentTime: Date | string = new Date(),
  settings?: Partial<ParkingSettings>
): boolean {
  ...
  if (contract.endDate) {
    const end = new Date(`${contract.endDate}T23:59:59`).getTime();
    if (!isNaN(end) && nowTime > end) return false;
  }

  // 24/7 or Weekly contracts are active at all times of the day
  if (contract.type === 'completo_24_7' || contract.type === 'semanal') {
    return true;
  }
  ...
  if (contract.type === 'nocturno') {
    // Default night schedule: 20:00 to 08:00 (8:00 PM to 8:00 AM next day)
    const { startMinutes, endMinutes } = parseScheduleHours(
      settings?.nightContractSchedule,
      20,
      8
    );

    if (startMinutes > endMinutes) {
      // Overnight schedule (e.g. 20:00 a 08:00)
      return currentMinuteOfDay >= startMinutes || currentMinuteOfDay < endMinutes;
    } else {
      return currentMinuteOfDay >= startMinutes && currentMinuteOfDay < endMinutes;
    }
  }

  if (contract.type === 'diurno') {
    // Default day schedule: 08:00 to 20:00 (8:00 AM to 8:00 PM)
    const { startMinutes, endMinutes } = parseScheduleHours(
      settings?.dayContractSchedule,
      8,
      20
    );
